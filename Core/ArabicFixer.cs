...
namespace ArabicUnityRTL.Core
{
    public static class ArabicFixer
    {
        ...
        private static string FixLine(string line)
        {
            ...
            foreach (var t in tokens)
            {
                if (t.Type == TokenType.Arabic) { hasArabic = true; break; }
            }
            if (!hasArabic) return line; // pure LTR line: nothing to do

            // Merge each maximal consecutive stretch of {Arabic, Tag} tokens
            // into one shaping/clustering unit before shaping+reversing it.
            // This matters whenever a TMP tag (color, sprite, etc.) sits
            // between two Arabic letters, or between a letter and its own
            // diacritic: shaping each Arabic token in isolation would treat
            // the tag as a hard boundary, orphaning a diacritic from its
            // base letter (it becomes a stray mark with nothing to attach
            // to) and blocking correct letter-joining across the tag. A
            // chain with no Arabic in it at all (e.g. two tags back-to-back
            // in a Latin sentence) is left completely untouched - it must
            // NOT go through this pipeline, since reversal should only ever
            // apply inside genuinely RTL content.
            ...
            while (ti < tokens.Count)
            {
                ...
                {
                    int start = ti;
                    while (ti < tokens.Count && (tokens[ti].Type == TokenType.Arabic || tokens[ti].Type == TokenType.Tag)) ti++;
                    ...
                    if (chainHasArabic)
                    {
                        string mergedShaped = ShapeAndReverseMixedRun(chain);
                        processed.Add(new Token(TokenType.Arabic, mergedShaped));
                    }
                    else
                    {
                        foreach (var ct in chain) processed.Add(ct);
                    }
                    continue;
                }
